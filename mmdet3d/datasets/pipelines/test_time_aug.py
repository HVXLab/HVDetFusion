...
class MultiScaleFlipAug3D(object):
    ...
    def __init__(self,
                 transforms,
                 img_scale=None,
                 pts_scale_ratio=None,
                 tta_flip=False,
                 flip_direction='horizontal',
                 bevflip_direction='horizontal',
                 bevflip=False,
                 bevrot=0,
                 bevscale_ratio=0,
                 tta_rot=0):
        ...
        self.flip = tta_flip
        self.bevflip = bevflip
        self.bevflip_direction = bevflip_direction
        self.rot = tta_rot if isinstance(tta_rot, list) else [tta_rot]
        self.bevrot=bevrot
        self.bevscale_ratio=bevscale_ratio
        # self.pcd_horizontal_flip = pcd_horizontal_flip
        # self.pcd_vertical_flip = pcd_vertical_flip
        self.bevrot = bevrot if isinstance(bevrot, list) else [bevrot]
        self.bevscale_ratio = bevscale_ratio if isinstance(bevscale_ratio, list) else [bevscale_ratio]
        self.flip_direction = flip_direction if isinstance(
            flip_direction, list) else [flip_direction]
        assert mmcv.is_list_of(self.flip_direction, str)
        if not self.flip and self.flip_direction != ['horizontal']:
    ...
    def __call__(self, results):
        ...
        bevflip_aug = [True] if self.bevflip else [False]
        # rot_aug = [True] if self.rot else [False]
        ...
        for flip in flip_aug:
            for direction in self.flip_direction:
                for rot in self.rot:
                    for bevrot in self.bevrot:
                        for bevscale_ratio in self.bevscale_ratio:
                            for bevflip in bevflip_aug:
                                ...
                                _results['flip_direction'] = direction
                                _results['rot'] = rot
                                _results['bevflip'] = bevflip
                                _results['bevflip_direction'] = self.bevflip_direction
                                _results['bevrot'] = self.bevrot
                                _results['bevscale_ratio'] = self.bevscale_ratio
