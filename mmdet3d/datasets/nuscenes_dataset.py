...
class NuScenesDataset(Custom3DDataset):
    ...
    def get_adj_info(self, info, index):
        info_adj_list = []
        adj_start, num_past, num_future, adj_step = self.multi_adj_frame_id_cfg
        for select_id in range(adj_start, num_past+num_future+1, adj_step):
            if select_id < num_past + 1:
                select_id = max(index - select_id, 0)
            else:
                select_id = min(index + select_id-num_past, len(self.data_infos) - 1)
    ...
    def _evaluate_single(self,
        ...
        nusc_eval.main(plot_examples=0, render_curves=render_curves)
    ...
    def evaluate(self,
        ...
        if isinstance(result_files, dict):
            ...
            for name in result_names:
                ...
                ret_dict = self._evaluate_single(result_files[name], render_curves=render_curves, score_thres=score_thres)
            results_dict.update(ret_dict)
        elif isinstance(result_files, str):
            results_dict = self._evaluate_single(result_files, render_curves=render_curves, score_thres=score_thres)
